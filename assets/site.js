          ...
          return function(cartItem) {
            return setTimeout(function() {
              if (theme.isHome && _this.$(".add-to-cart").hasClass("express")) {
                ...
                var mystring = productname.split(' - ');
                var j = mystring[0];
                var k = mystring[1];
                var quantity_product = $('input#quantity').val();
                var text_descr_quantity_1 = "Bestelle zwei weitere Strappys und spare 20%";
                var text_descr_quantity_2 = "Bestelle einen weiteren Strappy und spare 20%";
                _this.$(".product-quantity-pop").text(quantity_product);
                _this.$(".added-product-name").text(j);
                _this.$(".added-product-name-variant").text(k);
                _this.$(".product-variant-circle").addClass(k);
                _this.$("input[name='quantity']").removeClass("error");
                _this.$(".add-to-cart").removeClass("loading added-error").addClass("added-success");
                _this.$("#popup_added").addClass("show");
                if (quantity_product == "1") {
                _this.$(".text_descr_quantity").text(text_descr_quantity_1);
                }
                if (quantity_product == "2") {
                _this.$(".text_descr_quantity").text(text_descr_quantity_2);
                }
                if (!Modernizr.cssanimations) {
                  _this.addToCartButton.val(Theme.addToCartText);
                }
                if (_this.$("#popup_added").hasClass("show")){
                _this.$("#popup-close").click(function(){
                  _this.$("#popup_added").removeClass("show");
                  _this.$(".product-variant-circle").removeClass(k);
                });
                }
              }
              return _this.processing = false;
...
(function() {
  ...
  var top = $('.template-page .header-background').offset().top;
    top = $('.template-page .header-background').height() / 2 - 40;
  var left = $('.template-page .header-background').offset().left;
  $('.template-page h1.page-title').css({
      "position": "absolute",
      "width": "100%",
      "text-align": "center",
      "font-size": "56px",
